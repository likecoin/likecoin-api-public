...
const NFT_BOOK_LIKER_LAND_FEE_RATIO = 0.05;
const NFT_BOOK_TIP_LIKER_LAND_FEE_RATIO = 0.10;
const NFT_BOOK_LIKER_LAND_COMMISSION_RATIO = 0.3;
const NFT_BOOK_LIKER_LAND_ART_FEE_RATIO = 0.1;

function calculateStripeFee(inputAmount: number, currency = 'usd'): number {
...
describe('NFT Book Purchase - Language Normalization', () => {
  describe('normalizeLanguageForStripeLocale', () => {
    function normalizeLanguageForStripeLocale(language: string | undefined): string {
      switch (language) {
        case 'zh':
        case 'zh-Hant':
          return 'zh-TW';
...
describe('NFT Book Purchase - Check LikerLand Source', () => {
  describe('checkIsFromLikerLand', () => {
    function checkIsFromLikerLand(from: string): boolean {
...
describe('NFT Book Purchase - Item Price Calculations', () => {
  ...
  function calculateItemPrices(items: any[], from: string) {
