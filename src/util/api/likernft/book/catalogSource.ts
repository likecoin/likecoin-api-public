...
// Shared building blocks for the product-catalog feeds (Meta, Stripe). Each
// feed maps the same eligible books into its own schema, so keeping the fetch,
...
// (liker-land-v3 use-book-info.ts `brandName`), since the crawled page is
// compared against the feed. Returns author/publisher too so feeds can also
// expose them as their own labels.
function resolveCatalogBrand(book: NFTBookListingInfo): {
  author: string;
  publisher: string;
  brand: string;
} {
...
function isBookPriceInStock(price: NFTBookPrice): boolean {
  return price.isAutoDeliver || price.stock === undefined || price.stock > 0;
}

function getCatalogVariantTitle(baseTitle: string, price: NFTBookPrice): string {
...
// priceInDecimal is USD minor units (cents); the feeds want a display string
// of the form "<amount> USD".
function formatCatalogPriceUSD(price: NFTBookPrice): string {
...
function normalizeISBNToGTIN(isbn?: string): string | undefined {
  if (!isbn) return undefined;
  const gtin = isbn.replace(/\D/g, '');
  return gtin.length === 13 ? gtin : undefined;
}

// Meta limits a single catalog feed to 200k products; book listings are
// nowhere near that today, but cap defensively to keep the response bounded.
const CATALOG_MAX_BOOKS = 5000;
...
async function listCatalogEligibleBooks(): Promise<CatalogBook[]> {
...
// A single sellable variant, normalized once so the flat feeds (Meta, Stripe)
// are pure schema mappers and share one definition of which variants are valid
// (listed, positively priced, on a book with a name and image).
